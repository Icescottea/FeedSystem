...
export default function PaymentForm({ invoiceId, onSuccess }) {
  ...
  const [form, setForm] = useState({
    ...
    remainingDue: 0,           // display-only
    amountPaid: '',            // user can edit (defaults to remainingDue)
    paymentMethod: '',
    paymentDate: '',
    notes: ''
  });
    ...
    fetch(`${API_BASE}/api/invoices/${invoiceId}`)
      ...
      .then(inv => {
        ...
        setForm(prev => ({
          ...
          amountPaid: remaining.toString(), // default to remaining due
        }));
      })
      .catch(err => {
        console.error(err);
  ...
  return (
    <form onSubmit={submit} className="space-y-4 bg-white border rounded-md shadow p-6">
      ...
      {/* Remaining (read-only) */}
      <div>
        <label className="block mb-1 text-sm font-medium text-gray-700">Remaining Due (Rs.)</label>
        <input
          type="number"
          value={form.remainingDue}
          readOnly
          className="w-full border rounded-md px-3 py-2 text-sm bg-gray-100"
        />
      ...
      <div>
        <label className="block mb-1 text-sm font-medium text-gray-700">Amount to Pay (Rs.)</label>
        <input
          type="number"
          name="amountPaid"
          step="0.01"
