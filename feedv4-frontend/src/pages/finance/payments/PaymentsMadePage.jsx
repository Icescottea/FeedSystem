...
const PaymentsMadePage = () => {
  ...
  const filteredPayments = payments.filter(p => {
    const billNums = (p.billPayments || []).map(bp => bp.billNumber).join(' ');
    const matchesSearch = [p.paymentNumber, p.referenceNumber, p.vendorName, billNums]
      .filter(Boolean).some(f => f.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const totalPaid = paidPayments.reduce((s, p) => s + (parseFloat(p.paymentMade) || 0), 0);
  const totalUnused = payments.reduce((s, p) => s + (parseFloat(p.amountInExcess) || 0), 0);
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => navigate('/finance/payments/payments-made/new')}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Total Payments', value: payments.length, cls: 'text-gray-800' },
          { label: 'Paid Payments', value: paidPayments.length, cls: 'text-green-600' },
          { label: 'Total Amount Paid', value: `LKR ${totalPaid.toLocaleString()}`, cls: 'text-blue-600' },
          { label: 'Total Unused Amount', value: `LKR ${totalUnused.toLocaleString()}`, cls: 'text-orange-600' },
        ].map(card => (
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-4">
        <div className="flex flex-col md:flex-row gap-4">
          <input type="text" className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-500">
            {searchQuery || statusFilter !== 'all' ? 'No payments found matching your filters.' : 'No payments yet. Create your first payment!'}
          </div>
        ) : (
          <>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    {['Date','Payment Number','Reference','Vendor Name','Bills','Mode','Status','Amount','Unused','Actions'].map(h => (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {paginatedPayments.map(payment => {
                    const billNums = (payment.billPayments || []).map(bp => bp.billNumber).join(', ') || '—';
                    const excess = parseFloat(payment.amountInExcess) || 0;
                    return (
                      <tr key={payment.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-medium text-blue-600 cursor-pointer hover:text-blue-800"
                            onClick={() => navigate(`/finance/payments/payments-made/${payment.id}`)}>
                            {payment.paymentNumber}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{payment.referenceNumber || '—'}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                          {payment.vendorName || `Vendor #${payment.vendorId}`}
                        </td>
                        <td className="px-6 py-4 text-sm text-gray-600 max-w-[140px] truncate" title={billNums}>{billNums}</td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`text-sm font-medium ${excess > 0 ? 'text-orange-600' : 'text-gray-500'}`}>
                            LKR {excess.toLocaleString()}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <div className="flex gap-2">
                            <button onClick={() => navigate(`/finance/payments/payments-made/${payment.id}`)}
                              className="text-blue-600 hover:text-blue-800 font-medium">View</button>
                            <button onClick={() => navigate(`/finance/payments/payments-made/${payment.id}/edit`)}
